...
int main()
{
    ...
    cin >> s; 
    int Private_Key[6], incrementing_zeroes = 6 - (s.size() % 6);  
    int incrementing_zeroes_2 = incrementing_zeroes;
    ...
    for (int i = 0; i < 6; i++)
    {
        Private_Key[i] = rand() % (y - x + 1) + x;
        increasing_sum += Private_Key[i];
        x = increasing_sum + 1;
        y = x + 10; // incrementing upper limit by 10
    }
   ...
   cout<<"\n\t\t******************************************************************"<<endl;
    ...
    i = 0;
    int Cipher_Text[(s.size() / 6)]; // arr[3]
    int j = 0, c = 1, k = 0;
    int sum = 0;
    ...
    int arr_use_store[(s.size() / 6)];

    for (int i = 0; i < (s.size() / 6); i++)
    {
        arr_use_store[i] = (Cipher_Text[i] * inv_n) % m;
    }
    cout<<"\n\t\t******************************************************************"<<endl;
    cout<<"\t\tPSEUDO DECIPHERED TEXT :\t\t";
    for (int i = 0; i < (s.size() / 6); i++)
    {
        cout << arr_use_store[i] << " ";
    }
    ...
    while (1)
    {
        if (j == (s.size() / 6))
            break;
        string s1 = "000000";
        Backtracking(Private_Key, 6, arr_use_store[i], s1); 
        j++;
        i++;
